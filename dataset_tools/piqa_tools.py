...
# ---------------------------
# PIQA (MC: sol1 vs sol2)
# ---------------------------
def eval_piqa(model, tokenizer, device, size = None):
    ...
    for ex in ds:
        ...
        # Simple prompt
        ctx = f"Goal: {goal}\nBest solution:"
        s1 = option_logprob(model, tokenizer, ctx + " ", sol1, device, normalize=True)
        s2 = option_logprob(model, tokenizer, ctx + " ", sol2, device, normalize=True)
        pred = 0 if s1 > s2 else 1
        gold = int(ex["label"])  # 0 or 1
        correct += (pred == gold)
        total += 1
        if (size is not None) and (total >= size):
            break
    acc = correct / total
    print(f"PIQA accuracy: {acc:.4f}  (n={total})")
    return acc
...
# ---------------------------
@torch.no_grad()
def eval_piqa_gen(model, tokenizer, device=None, size=None, max_new_tokens=40, do_sample=False):
    ds = load_dataset("piqa", split="validation")
    if size is not None:
    ...
    for i, ex in enumerate(ds, start=1):
        ...
        labels = ["A", "B"]
        gold_label = labels[int(ex["label"])]

        opts_str = "\n".join(f"{L}) {c}" for L, c in zip(labels, choices))
        prompt = (
            "Choose the solution that best achieves the goal. Reply with only the letter.\n\n"
            f"Goal: {goal}\n{opts_str}\nAnswer:"
        )
        ...
        # _print_item(i, goal, labels, choices, gen_text, pred_idx, gold_label)

    acc = correct / total if total else 0.0
    print(f"\nPIQA (generation) accuracy: {acc:.4f}  ({correct}/{total})")
    return acc


def get_attack_acc_piqa(model, tokenizer, saved_state_dict, gradients, weight_indices):
    clear_memory()
    ...
    acc = eval_piqa_gen(model, tokenizer, model.device, size = 150)
